...
func _ready():
	startLevel()
	$NoteTaker.global_position.y = 50

func _process(delta):
	if ($StartLabel.visible):
		$NoteTaker.global_position.y += 30 * delta
	elif (!$StartLabel.visible and !spawningFinished):
		$NoteTaker.spriteChange()
...
func noEnemies():
	if (get_node_or_null("NoteTaker") != null):
		return false
	else:
		return true
...
func nextLevel():
	...
	if (player != null):
		player.saving()
	var done = gameOverScreen.instance()
	var doneTitle = done.get_child(1)
	doneTitle.text = "YOU WIN!"
	doneTitle.rect_global_position.x += 10
	$OnTop.add_child(done)
